...
1. 目前无法隔离人格的 Skills。
2. 传统 handoff 式 SubAgent 运行不会保留独立的持久化对话历史；通过运行时工具创建的持久化 SubAgent 实例会保留持久化对话状态以及后台任务状态。

## 持久化后台运行

持久化 SubAgent 实例支持后台提交任务。调用 `run_subagent(name="analyst", input="...", background_task=true)` 时，工具会立即返回，结果中包含 `task_id` 和排队状态，而不是等待最终回复。

你可以使用 `get_subagent_status(name="analyst")` 查询实例当前是否忙碌，以及最近一次后台任务的状态、最终结果和近期事件。单个持久化实例在同一时间最多只会运行一个活动任务；如果需要真正的并行执行，请创建多个持久化实例分别运行。
